...
class Physics
{
	...
	public static function createDynamicBodyFromSpriteBoundingBox(S:FlxSprite,M:Float,IsDynamic:Bool = true,HasSensor:Bool = false):B2Body
	{
		...
		if (HasSensor)
		{
			...
			hitShape.setAsBox(Physics.screenToWorld(S.width/1.8), Physics.screenToWorld(S.height/0.33));
			...
			hitShape.setAsBox(Physics.screenToWorld(S.width/1), Physics.screenToWorld(S.height/1.8));
