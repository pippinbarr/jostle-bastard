...
class Seat extends PhysicsSprite
{
	...
	private function handleImpactStrength(impulse:Float,other:PhysicsSprite):Void
	{
		...
		else if (impulse > 15 && Math.random() > 0.2)
		{
			getUp();
		}
		else if (impulse > 2 && Math.random() > 0.8)		
		{
			getUp();
		}	
	}
	...
	// 		hColour = GameAssets.HIGHLIGHT_REPLACEMENT;
	// 	}
